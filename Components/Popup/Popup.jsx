...
export default function Popup({ btnTxt = "Popup", popupContentBoxMaterialHeading = "This is a Popup", popupContentBoxMaterialPara = material, popupContentBoxMaterialButton = "Download", userPopupStyle, userPopupContentBox,userCancelBtnStyle,userButtonStyle, userPopupContentStyle, userPopupContentBoxMaterialStyle }) {

    const [showPopup, popupRef, popupState, closePopup] = usePopup(false);

    return (
        <section className={`${popupStyle.popup} ${userPopupStyle}`}>
            <Button buttonStyle={userButtonStyle}  buttonText={btnTxt} onclick={() => popupState(true)}  />
            {
                showPopup &&
                <div className={`${popupStyle.popupContent} ${userPopupContentStyle}`}>
                    <div className={`${popupStyle.popupContentBox} ${userPopupContentBox}`} onClick={closePopup} ref={popupRef}>
                        <span className={`${popupStyle.cancelBtn} ${userCancelBtnStyle}`} onClick={() => popupState(false)}>
                        ...
                        <div className={`${popupStyle.popupContentBoxMaterial} ${userPopupContentBoxMaterialStyle}`}>
                            ...
                            <Button buttonStyle={userButtonStyle} buttonText={popupContentBoxMaterialButton}></Button>
