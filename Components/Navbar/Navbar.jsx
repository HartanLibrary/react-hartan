...
export default function Navbar({ navList = list, logoImgSrc = "https://dummyimage.com/374x416", logoName = "Hartan", headerStyle, sideBarStyle, navListStyle, logoStyle, buttonStyle, buttonText = "Light / Dark", statesObj = states }) {
    ...
    return (
        <header className={`${headerStyle}`}>
            ...
            {
                statesObj.logo.full && (
                    <div className={`${navbarStyles.logo} ${logoStyle}`}>
...

            {
                sidebarStatus ?
                    (
                        <nav className={`${navbarStyles.sidebar} ${sideBarStyle}`}>
                            ...
                            <ul>
                                {
                                    navList?.map((listItem, id) => {
                                        return <li key={id} onClick={closeSidebar} className={`${navListStyle}`}>
                        ...
                        <nav className={`${navbarStyles.navbar}`}>
                            <ul>
                                {
                                    navList?.map((listItem, id) => {
                                        return <li key={id} className={`${navListStyle}`}><a href={listItem.href}>{listItem.text}</a></li>
                                    })
                                }
                            </ul>
                        </nav>
...

            {
                ...
                <Button buttonStyle={`${buttonStyle}`} buttonText={buttonText}></Button>
            }
        </header>
    )
}
